...
BOOL CTestResultQueryDlg::OnInitDialog()
{
	...
	m_List.InsertColumn(1,"TIME",LVCFMT_LEFT,200,6);
	m_List.InsertColumn(7,"STATE",LVCFMT_LEFT,102,7);
	//!连接数据库
...
void CTestResultQueryDlg::AddToList(int flag)   // flag 1: 向后查询; 0: 向后查询
{
	...
	if(flag==1)
	{
		while(count<LIST_MAXITEM)
		{
			...
			m_List.SetItemText(0,1,(char*)(_bstr_t)m_Ado.m_pRecordset->GetCollect("SBI_TIME"));
			m_List.SetItemText(0,2,(char*)(_bstr_t)m_Ado.m_pRecordset->GetCollect("SBI_RFID_STATE"));
...
void CTestResultQueryDlg::OnNMClickList1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	m_strATW  = m_List.GetItemText(m_pos,2);//ATW
	if(m_strATW=="0")
	{
	  m_strATW = "未写";
	}
