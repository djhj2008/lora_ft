...
void CTestSettingsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MaxChars(pDX, m_strTCCCode, 9);
	DDX_Text(pDX, IDC_EDIT_FACTORY_CODE, m_strFactoryCode);
	DDV_MaxChars(pDX, m_strFactoryCode, 5);
}
...
void CTestSettingsDlg::SaveSmallTestToolConfig(void)
{
 ...
 //   ::WritePrivateProfileString(_T("SmallBJConfig"), _T("ShutDownCurrentMax"), m_strShutDownCurrentMaxValue, strPath);
    ...
    ::WritePrivateProfileString(_T("SmallBJConfig"), _T("TCCCode"), m_strTCCCode, strPath);
    ::WritePrivateProfileString(_T("SmallBJConfig"), _T("FactoryCode"), m_strFactoryCode, strPath);
